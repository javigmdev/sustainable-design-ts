...
describe('Either', () => {
  ...
  describe('Either - Functor and Monad Laws', () => {
    ...
    test('Functor Law - Composition', () => {
      const either = Either.right(5);
      expect(either.map((x) => f(g(x))).fold(identity, identity)).toEqual(
        either.map(g).map(f).fold(identity, identity)
      );
    });
    ...
    test('Monad Law - Left Identity', () => {
      ...
      expect(either.flatMap(fEither).fold(identity, identity)).toEqual(fEither(value).fold(identity, identity));
    });

    test('Monad Law - Right Identity', () => {
      const either = Either.right(5);
      expect(either.flatMap(Either.right).fold(identity, identity)).toEqual(either.fold(identity, identity));
    });

    test('Monad Law - Associativity', () => {
      const either = Either.right(5);
      expect(either.flatMap(fEither).flatMap(gEither).fold(identity, identity)).toEqual(
        either.flatMap((x) => fEither(x).flatMap(gEither)).fold(identity, identity)
      );
    });
  });
});
